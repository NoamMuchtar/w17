...
const SeriesDetails: FunctionComponent<SeriesDetailsProps> = () => {
  ...
  const seriesIndex = series.findIndex((ser) => ser.name == name);
  return (
    <>
      <h4 className="display-4">{name}</h4>
      <img src={series[seriesIndex].image} alt={series[seriesIndex].name} />
      <p>Description: {series[seriesIndex].description}</p>
      <p>Raiting: {series[seriesIndex].rate}</p>
      <p>start Year: {series[seriesIndex].startYear}</p>
      <p>seasons: {series[seriesIndex].seasons}</p>
    </>
  );
};
